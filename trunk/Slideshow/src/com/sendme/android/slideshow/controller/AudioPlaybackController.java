...
extends Controller
implements OnCompletionListener, HeartbeatTaskEventListener, UIEventListener
{
	...
	public void playCurrentAudioPlaylistEntry()
	{
		...
		{
			// This is a last resort, because it isn't always picking up some exceptions
			log.error("Exception switching track.", e);
	...
	{
		// Ok, first we need to figure out which track we're going to play.  Let's ask
		// the handy-dandy playlist manager.
		try
		{
			Integer currentId = settingsManager.getCurrentAudioPlaylistEntryId();

			AudioPlaylistEntry nextEntry = audioPlaylistManager.getNextEntry(currentId);

			log.info("About to play next song: " + nextEntry + " -- " + currentId);

			if (nextEntry != null && !nextEntry.getId().equals(currentId))
			{
				settingsManager.setCurrentAudioPlaylistEntryId(nextEntry.getId());
	...
	public void playPreviousAudioTrack()
	{
		...
		try
		{
			...
			AudioPlaylistEntry previousEntry = audioPlaylistManager.getPreviousEntry(currentId);

			log.info("About to play previous song: " + previousEntry + " -- " + currentId);

			if (previousEntry != null && !previousEntry.getId().equals(currentId))
			{
				settingsManager.setCurrentAudioPlaylistEntryId(previousEntry.getId());
