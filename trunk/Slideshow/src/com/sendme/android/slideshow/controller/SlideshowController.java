...
extends Controller
implements HeartbeatTaskEventListener, UIEventListener, BackgroundImageLoaderListener
{
	...
	{
		// Ok, first we need to figure out which photo we're going to show.  Let's ask
		...
		try
		{
			...
			log.info("About to try next image: " + nextEntry + " -- " + currentId);

			// If it's not null, and its not the same entry we have, we can load it
			if (nextEntry != null)
			{
				if (!nextEntry.getId().equals(currentId))
				{
					settingsManager.setCurrentSlideshowEntryId(nextEntry.getId());
	...
	public void loadPreviousPhoto(Long time)
	{
		...
		try
		{
			...
			SlideshowEntry previousEntry = slideshowManager.getPreviousEntry(currentId);

			log.info("About to try previous image: " + previousEntry + " -- " + currentId);
			...
			if (previousEntry != null)
			{
				...
				{
					settingsManager.setCurrentSlideshowEntryId(previousEntry.getId());
	...
	public void setImageLoaderTaskEventListenerIdentifier(Integer imageLoaderTaskEventListenerIdentifier)
	{
