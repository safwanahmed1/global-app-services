 ...
 */
public class InitializationTask extends
		AsyncTask<InitializationPhase, InitializationEventType, Void> {
	...
	protected Void doInBackground(InitializationPhase... params) {
		...
		case LOCAL_IMAGE_SOURCE: {
			...
			if (settingsManager.isUseLocalImages()) {
				try {
					...
					localImageSource.checkForUpdates(
		...
		case FACEBOOK_IMAGE_SOURCE: {
			// Sadly, there isn't much we can do here. The Facebook lifecycle
			// just kills us,
			// really. So, we're going to simply set it active or not, and then
			// let the main
		...
		case LOCAL_AUDIO_TRACKS_SOURCE: {
			...
			if (settingsManager.isPlayMusic()) {
				try {
					...
					localAudioTrackSource.checkForUpdates(
							System.currentTimeMillis(), false);
	...
	public void setLocalAudioTrackSource(
			LocalAudioTrackSource localAudioTrackSource) {
